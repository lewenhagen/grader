...

{
    ...
    if ($1 == "Kmom10 Tentamen ") {
        #print $1, $2, $3
        $1 = "Tentamen"
        $3 = "ok"
    }
    if ($1 == "Tentamen") print $3
    result[$2][$1][$3]++
}
...
END {
    for (name in result) {
        for (kmom in result[name]) {
            ...
            #if (kmom == "Kmom10 Tentamen ") {
            total_row[kmom]["Tentamen"] += result[name][kmom]["ok"]
            #}
            total_row[kmom]["total_sum"] += (total_row[kmom]["G"] + total_row[kmom]["Ux"])


            # Add to teacher row, sums of (total, G, Ux)
            result[name][kmom]["total"] += (result[name][kmom]["G"] + result[name][kmom]["Ux"] + result[name][kmom]["Tentamen"])
    ...
    # Print header row
    printf ("%-20s%s%10s%10s%10s%10s%10s%10s%10s%10s%s%8s%7s%6s%7s\n", "Name", "|", "Kmom01", "Kmom02", "Kmom03", "Kmom04", "Kmom05", "Kmom06", "Tentamen", "Kmom10", "|", "Totalt", "%", "G", "%G")
    printf ("%s\n", line)
    ...
    for (name in result) {
        ...
        printf ("%-20s%s%10s%10s%10s%10s%10s%10s%10s%s%10s%8s%6.0f%%%6s%6.0f%%\n",
            name,
            "|",
